...
`proposal_validation.required_for_harness_components=true` requires skills, rules, routing, eval-policy, architecture-gate, and frozen-boundary proposals to pass an isolated baseline/candidate comparison before human acceptance. `minimum_improvement` controls the score delta required for `validated`; `regression_tolerance` protects regression and sealed sets. Copy workspaces provide project-state isolation, not an operating-system sandbox, so every command case is still checked by `SafetyPolicy` and must match a reviewed command prefix.
