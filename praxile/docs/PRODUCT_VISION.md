...
P2 deepens that loop by treating experience quality as a product surface: proposals carry evidence, confidence, scope, and anti-scope; skills have lifecycle metadata and version snapshots; failure patterns are structured guardrails; routing stats can be summarized from trajectories; consolidation remains proposal-only.
