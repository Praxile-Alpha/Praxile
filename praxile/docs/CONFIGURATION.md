...
Praxile uses project-local JSON config at `.praxile/config.json`.
...
The repository includes `praxile.config.example.json` as a reference for the generated state shape, safety policy, gateway settings, model roles, and OpenClaw-inspired channel bindings.

## Init Detection

`praxile init` detects Python, Node/React/Vite/Next, Go, and Rust markers. For new projects it seeds `runtime.default_test_commands`; `praxile doctor` then compares configured, suggested, and active verification commands, reports missing tools, and checks SQLite/FTS index health plus project-map coverage.
...
## Runtime, Search, Index, And Transport

Key P1 knobs live in `.praxile/config.json`:
...
`search.backend=auto` prefers `rg` and falls back to Python search, with sensitive-file filtering and skipped/error statistics in observations. Project map caching stores a short-lived summary in `.praxile/cache/project_map.json`; use `{"type":"project_map","refresh":true}` when recent filesystem changes matter. `praxile index status` checks expected assets, indexed assets, FTS availability, missing rows, and stale rows; `praxile index rebuild` syncs by content hash, repairs missing FTS rows, and removes stale rows without clearing the whole asset table. The default model transport is dependency-free `urllib`; install `praxile[http]` to allow optional `httpx`. Both transports expose `stream_json` for SSE-style model/event streams.
...
`task_analysis.llm_assisted=false` keeps classification deterministic. Set it to `true` to call `llm_model_role`, such as `cheap_model`, `review_recommendation`, or `planning_model`, for lightweight intent recognition. LLM analysis can raise risk or add UI/privacy signals, but accepted frozen boundaries remain hard gates.
...
`evolution.llm_assisted_proposals=false` keeps proposal generation deterministic. Set it to `true` to let `model_roles.proposal_composer` or the legacy `evolution_model` route propose additional experience assets. LLM-assisted proposals must cite evidence, include confidence plus scope/anti-scope, target only safe `.praxile/` paths, and remain pending until accepted.
...
`architecture_gate.shadow_mode=true` records a dry-run shadow plan after a gate is triggered, but it still does not land file edits or implementation commands. The default remains a hard stop.
...
Run feedback updates `user_feedback_reward` and `final_reward`. Proposal feedback can lower confidence and change recommendation to `reject_or_edit`. Negative asset feedback increments negative outcome counts and creates a proposal-only lifecycle review rather than silently rewriting durable assets. `feedback auto` can split one natural-language note into run, proposal, asset, or pattern feedback while keeping durable negative asset changes governed.
...
```bash
praxile channel bind telegram -1001234567890 --name team-alerts --mode bidirectional --token-env TELEGRAM_BOT_TOKEN --free-response
```

Bind Discord:

```bash
praxile channel bind discord 123456789012345678 --guild-id 987654321098765432 --name dev-room --mode task --token-env DISCORD_BOT_TOKEN --auto-thread
```
...
Current boundary: Praxile implements channel configuration, binding management, gateway introspection, and local route metadata. Production Telegram/Discord bot listeners can be added on top of the same config without changing `.praxile/` state semantics.
