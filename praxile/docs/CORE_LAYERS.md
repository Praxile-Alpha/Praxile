...
`praxile.inspector` detects stack markers for `praxile init` and `praxile doctor`. It suggests verification commands and reports missing tools without executing project code.
...
It supports task submission, latest review, pending proposal queue listing, proposal accept/reject, history, memory retrieval, and channel listing.
