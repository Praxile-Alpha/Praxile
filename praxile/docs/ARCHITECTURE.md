...
Runtime loads only `active` skills. Deprecated or replaced skills remain auditable on disk and in proposal history but are removed from runtime retrieval.

Failure patterns include a structured type, status, confidence, scope, and anti-scope so repeated safety, regression, model, environment, architecture, UX, or task failures can become searchable guardrails without turning one run into a universal rule.
...
`praxile models --stats` aggregates route performance by task type and target from recorded trajectories. These stats are advisory; Praxile still changes routing only through reviewed config edits or accepted proposals.
