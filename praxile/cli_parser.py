...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_eval_ab.add_argument("--capability-protocol", default=None, help="P2-A goal, proxy, firewall and terminal rule JSON")
    p_eval_ab.add_argument("--proxy-eval", default=None, metavar="PROXY_ID@VERSION", help="Approved development-only proxy eval")
    p_eval_ab.add_argument("--development-only", action="store_true", help="Run a proxy-development A/B without held-out eligibility")
    ...
    p_proxy_propose = eval_sub.add_parser("proxy-propose", help="Register an immutable agent-authored proxy eval proposal")
    p_proxy_propose.add_argument("spec", help="ProxyEvalProposal JSON")
    p_proxy_propose.set_defaults(func=cmd_eval_proxy_propose)
    p_proxy_review = eval_sub.add_parser("proxy-review", help="Inspect a registered proxy eval")
    p_proxy_review.add_argument("proxy_id")
    p_proxy_review.add_argument("--version", required=True)
    p_proxy_review.set_defaults(func=cmd_eval_proxy_review)
    p_proxy_approve = eval_sub.add_parser("proxy-approve", help="Interactively approve a proxy eval")
    p_proxy_approve.add_argument("proxy_id")
    p_proxy_approve.add_argument("--version", required=True)
    p_proxy_approve.add_argument("--reviewer", required=True)
    p_proxy_approve.set_defaults(func=cmd_eval_proxy_approve)
    p_proxy_run = eval_sub.add_parser("proxy-run", help="Score an approved proxy eval on development A/B evidence")
    p_proxy_run.add_argument("proxy_id")
    p_proxy_run.add_argument("--version", required=True)
    p_proxy_run.add_argument("--experiment-id", required=True)
    p_proxy_run.add_argument("--json", action="store_true")
    p_proxy_run.set_defaults(func=cmd_eval_proxy_run)
    p_eval_capability = eval_sub.add_parser("capability-check", help="Validate a P2-A capability protocol without running a model")
    p_eval_capability.add_argument("protocol", help="Versioned capability protocol JSON")
    p_eval_capability.add_argument("--json", action="store_true")
    p_eval_capability.set_defaults(func=cmd_eval_capability_check)
    ...
    p_harness_candidate_rollback.add_argument("--executor-profile", default="default")
    p_harness_candidate_rollback.add_argument("--task-family", default="default")
    p_harness_candidate_rollback.set_defaults(func=cmd_harness_candidate_rollback)
    p_harness_lab_run = harness_sub.add_parser("lab-run", help="Execute a frozen Harness Candidate in repeated dev/held-out isolation")
    p_harness_lab_run.add_argument("manifest")
    p_harness_lab_run.add_argument("--development-tasks", required=True)
    p_harness_lab_run.add_argument("--heldout-tasks", required=True)
    p_harness_lab_run.add_argument("--resume", action="store_true")
    p_harness_lab_run.add_argument("--keep-workspaces", action="store_true")
    p_harness_lab_run.add_argument("--source", action="append", default=[], metavar="REPO=PATH")
    p_harness_lab_run.add_argument("--json", action="store_true")
    p_harness_lab_run.set_defaults(func=cmd_harness_lab_run)
    p_harness_lab_matrix = harness_sub.add_parser("lab-matrix", help="Build a multi-executor compatibility matrix from Harness Lab reports")
    p_harness_lab_matrix.add_argument("report", nargs="+")
    p_harness_lab_matrix.add_argument("--output", default=None)
    p_harness_lab_matrix.set_defaults(func=cmd_harness_lab_matrix)
    p_harness_lab_digest = harness_sub.add_parser("lab-digest", help="Compute the immutable digest for a Harness Lab task-set JSON")
    p_harness_lab_digest.add_argument("task_set")
    p_harness_lab_digest.add_argument("--json", action="store_true")
    p_harness_lab_digest.set_defaults(func=cmd_harness_lab_digest)
