...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="praxile",
        description="Praxile: local code-project self-evolution harness",
    )
    ...
    p_demo = sub.add_parser("demo", help="Create and run a local self-evolution demo project")
    ...
    p_consolidate.add_argument("--conflicts", action="store_true", help="Inspect assets with possible conflicting guidance")
...
def cmd_init(args: argparse.Namespace, project_root: Path) -> int:
    ...
    if not has_configured_models(config):
        ...
        print("3. Explain the self-evolution loop: praxile explain latest")
...
def run_fast_demo(args: argparse.Namespace, demo_root: Path) -> int:
    ...
    print("[5/6] Generating reward and proposals")
    trajectory = logger.finish(status="completed", summary="Recorded a fast calculator repair self-evolution trajectory.")
    report = RewardEngine(config).build_report(trajectory, after_results)
...
def cmd_consolidate(args: argparse.Namespace, project_root: Path) -> int:
    ...
    explicit = any(
        ...
        for name in ["duplicates", "stale", "conflicts", "low_value"]
    )
    checks = {
